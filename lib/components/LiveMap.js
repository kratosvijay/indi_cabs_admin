...
export default function LiveMap({ rides, drivers }) {
  ...
  // Define icons inside the component or a useMemo to ensure Leaflet is loaded
  ...
  return (
    <div className="map-wrapper glass">
      ...
      >
        ...
        {rides?.filter(r => r.pickupLocation?.latitude && r.pickupLocation?.longitude).map(ride => (
          <Marker 
            key={ride.id} 
        ...
        <ChangeView center={
          (rides?.[0]?.pickupLocation?.latitude && rides?.[0]?.pickupLocation?.longitude) 
            ? [rides[0].pickupLocation.latitude, rides[0].pickupLocation.longitude] 
            : null
        } />
      </MapContainer>
